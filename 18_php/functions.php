...
{
    global $db;
    mysqli_query($db, "DELETE FROM website WHERE DataID = $id");
    return mysqli_affected_rows($db);
}
...
{
    global $db;
    //ambil data dari login.php
    $userN = strtolower(stripslashes($data["username"]));
    $password = mysqli_real_escape_string($db, $data["password"]);
    $password2 = mysqli_real_escape_string($db, $data["password2"]);

    //cek username apakah ada atau tidak
    $result = mysqli_query($db, "SELECT DataUsername FROM  users WHERE DataUsername = '$userN' ");
    ...
    $query = "INSERT INTO users VALUES('','$userN','$password'
)";
    mysqli_query($db, $query);
